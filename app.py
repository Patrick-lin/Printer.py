...
def recieve_trans(decoded_data):
    ...
    # 数据过滤：
    documentNO = re.sub(r'[^A-Z0-9]', '', userInfo.get('documentNO'))
    nationality = re.sub(r'[^A-Z0-9]', '', userInfo.get('nationality'))
    if nationality == "D":
        nationality = "DEU"
    result = {
...
def insertDatabase(data):
    ...
    for db_config in databases:
        try:
            ...
            # 数据再过滤一遍
            documentNO = re.sub(r'[^A-Z0-9]', '', data.get('documentNO'))
            nationality = re.sub(r'[^A-Z0-9]', '', data.get('nationality'))
            logging.info(f"Now DocumentNo：{documentNO}")
            if nationality == "D":
                nationality = "DEU"
